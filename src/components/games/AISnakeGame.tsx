...
const AISnakeGame: React.FC<GameEngineProps> = ({ 
  ...
  const makeAIMove = useCallback((state: SnakeGameState) => {
    ...
    possibleDirections.forEach(direction => {
      ...
      if (aiSnake.some((segment: SnakeSegment) => segment.x === newHead.x && segment.y === newHead.y)) {
        return;
      }
      
      // Check player collision (competitive mode)
      if (playerSnake.some((segment: SnakeSegment) => segment.x === newHead.x && segment.y === newHead.y)) {
        return;
      }
      ...
      const foodDistance = Math.abs(newHead.x - food.x) + Math.abs(newHead.y - food.y);
      ...
      const playerDistance = Math.abs(newHead.x - playerHead.x) + Math.abs(newHead.y - playerHead.y);
  ...
  const updateGame = useCallback((timestamp: number) => {
    ...
    if (!snakeGameState.snake || !snakeGameState.direction || snakeGameState.gameOver || snakeGameState.paused) return;
    ...
    setGameState((prevState) => {
      ...
      if (head.x < 0 || head.x >= CANVAS_WIDTH / GRID_SIZE ||
        ...
        onGameEvent({ type: 'end', data: { score: newState.score, winner: 'ai' } });
        return newState;
      }
      
      // Check self collision
      if (newState.snake.some((segment: SnakeSegment) => segment.x === head.x && segment.y === head.y)) {
        newState.gameOver = true;
        onGameEvent({ type: 'end', data: { score: newState.score, winner: 'ai' } });
        return newState;
      }
      ...
      if (aiHead.x < 0 || aiHead.x >= CANVAS_WIDTH / GRID_SIZE ||
          aiHead.y < 0 || aiHead.y >= CANVAS_HEIGHT / GRID_SIZE) {
        newState.gameOver = true;
        onGameEvent({ type: 'end', data: { score: newState.score, winner: 'player' } });
        return newState;
      }
      
      // Check AI self collision
      if (newState.aiOpponent.snake.some((segment: SnakeSegment) => segment.x === aiHead.x && segment.y === aiHead.y)) {
        newState.gameOver = true;
        onGameEvent({ type: 'end', data: { score: newState.score, winner: 'player' } });
        return newState;
      }
      ...
      if (newState.snake.some((segment: SnakeSegment) => segment.x === aiHead.x && segment.y === aiHead.y) ||
          newState.aiOpponent.snake.some((segment: SnakeSegment) => segment.x === head.x && segment.y === head.y)) {
        newState.gameOver = true;
        onGameEvent({ type: 'end', data: { score: newState.score, winner: 'tie' } });
        return newState;
      }
      ...
      if (head.x === newState.food.x && head.y === newState.food.y) {
        ...
        onGameEvent({ type: 'score', data: { score: newState.score, points: newState.food.value } });
      ...
      if (aiHead.x === newState.food.x && aiHead.y === newState.food.y) {
        ...
        if (timePlaying > 30) { // After 30 seconds, AI starts adapting
          newState.aiOpponent.behavior.aggression = Math.min(1, newState.aiOpponent.behavior.aggression + 0.05);
          newState.aiOpponent.behavior.adaptation = Math.min(1, newState.aiOpponent.behavior.adaptation + 0.03);
        }
  ...
  useEffect(() => {
    ...
    const handleKeyPress = () => {
      ...
      if (controls.keyboard['ArrowUp'] || controls.keyboard['w']) {
        ...
        setGameState((prev) => ({ ...prev, paused: !(prev as SnakeGameState).paused }));
        return;
      }
      
      if (newDirection.x !== snakeGameState.direction.x || newDirection.y !== snakeGameState.direction.y) {
        setGameState((prev) => ({ 
          ...prev, 
  ...
  useEffect(() => {
    ...
    if (!canvas.current || !snakeGameState.snake || !snakeGameState.food) return;
    ...
    // Draw AI snake
    snakeGameState.aiOpponent.snake.forEach((segment: SnakeSegment, index: number) => {
    ...
    // Draw food
    ctx.fillStyle = snakeGameState.food.type === 'special' ? '#ffd700' : '#4caf50';
    ctx.fillRect(
    ...
    ctx.fillText(`AI Aggression: ${Math.round(snakeGameState.aiOpponent.behavior.aggression * 100)}%`, CANVAS_WIDTH - 200, 30);
    ctx.fillText(`AI Adaptation: ${Math.round(snakeGameState.aiOpponent.behavior.adaptation * 100)}%`, CANVAS_WIDTH - 200, 50);
    ...
    if (snakeGameState.gameOver) {
      ...
      let winner = 'Tie!';
      if (snakeGameState.score > snakeGameState.aiOpponent.score) winner = 'Player Wins!';
      else if (snakeGameState.aiOpponent.score > snakeGameState.score) winner = 'AI Wins!';
  ...
  useEffect(() => {
    ...
    if (snakeGameState.snake && snakeGameState.direction && !snakeGameState.gameOver) {
      animationFrameRef.current = requestAnimationFrame(updateGame);
    }

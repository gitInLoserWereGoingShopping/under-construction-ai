...
interface AIAssistantProps {
  // Component can be configured if needed
}
...
const experiments: AIExperiment[] = [
  {
    ...
    description: 'AI writes code based on your dreams. Describe a dream, get functional code.',
    icon: '🌙',
    category: 'text',
    weirdness: 7,
    status: 'experimental'
  },
  {
    ...
    description: 'Transform emotions into hex colors and generate mood palettes.',
    icon: '🎨',
    category: 'visual',
    weirdness: 5,
    status: 'stable'
  ...
  },
  {
    ...
    description: 'An AI that provides therapy to other AI systems. Meta-healing.',
    icon: '🤖',
    category: 'text',
    weirdness: 10,
    status: 'forbidden'
  },
  {
    ...
    description: 'Describes what colors and flavors sound like. Upload audio, get taste profiles.',
    icon: '👅',
    category: 'audio',
    weirdness: 8,
    status: 'experimental'
  },
  {
    ...
    description: 'Chat with AI versions of yourself from different time periods.',
    icon: '⏰',
    category: 'text',
    weirdness: 9,
    status: 'chaotic'
...
const ExperimentCard = styled.div<{ $weirdness: number; $status: string }>`
  ...
  border: 2px solid ${props => 
    props.$status === 'forbidden' ? '#ff4757' :
    props.$status === 'chaotic' ? '#ff6348' :
    props.$status === 'experimental' ? '#ffa502' :
    '#2ed573'
  };
  border-radius: 15px;
  padding: 2rem;
  ...
  position: relative;
  overflow: hidden;
  animation: ${props => props.$weirdness > 8 ? css`${float} 3s ease-in-out infinite` : 'none'};
  
  &:hover {
    transform: ${props => props.$status === 'forbidden' ? 
      `scale(1.05) ${props.$weirdness > 9 ? 'rotate(1deg)' : ''}` : 
      'translateY(-10px) scale(1.02)'};
    box-shadow: 0 20px 40px rgba(0, 0, 0, 0.4);
    border-color: ${props => 
      props.$status === 'forbidden' ? '#ff3742' :
      props.$status === 'chaotic' ? '#ff5438' :
      props.$status === 'experimental' ? '#ff9502' :
      '#25d366'
    };
    
    ${props => props.$status === 'forbidden' && css`
      animation: ${glitch} 0.3s ease-in-out;
    `}
  }

  ${props => props.$status === 'chaotic' && css`
    &::before {
      content: '';
      position: absolute;
      top: -50%;
...
const ExperimentIcon = styled.div<{ $weirdness: number }>`
  font-size: 3rem;
  margin-bottom: 1rem;
  animation: ${props => props.$weirdness > 7 ? css`${pulse} 2s ease-in-out infinite` : 'none'};
`;
...
const WeirdnessMeter = styled.div<{ $level: number }>`
  ...
  span {
    width: 8px;
    height: 20px;
    background: ${props => props.$level > 8 ? '#ff4757' : props.$level > 5 ? '#ffa502' : '#2ed573'};
    border-radius: 2px;
    opacity: ${props => props => props.$level / 10};
  }
`;
  ...
  return (
    <LabContainer>
      <LabHeader>
        ...
        <LabSubtitle>
          Welcome to the playground for weird AI experiments. 
          Where logic meets chaos and beautiful things happen.
          Pick an experiment and let's break some rules together.
        </LabSubtitle>
      </LabHeader>
      ...
      {!selectedExperiment ? (
        ...
        <ActiveExperiment>
          <h2>🧪 {selectedExperiment.icon} {selectedExperiment.name}</h2>
